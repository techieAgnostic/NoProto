...
use crate::{
    json_decode,
    json_flex::JSMAP,
    pointer::NP_Cursor_Parent,
    schema::{NP_Bytes_Data, NP_Map_List_Data, NP_String_Data, NP_Struct_Data, NP_Tuple_Data},
};
use crate::{memory::NP_Memory, utils::opt_err};
use crate::{schema::NP_TypeKeys, NP_Size_Data};
use alloc::boxed::Box;
...
use crate::{collection::struc::NP_Struct, schema::NP_Parsed_Schema};
use alloc::vec::Vec;
...
impl NP_Buffer {
    #[doc(hidden)]
    pub fn _new(memory: NP_Memory) -> Self {
        // make new buffer
    ...
    pub fn json_encode(&self, path: &[&str]) -> Result<NP_JSON, NP_Error> {
        let value_cursor =
            NP_Cursor::select(&self.memory, self.cursor.clone(), false, false, path)?;
        ...
        if let Some(x) = value_cursor {
            ...
            json_map.insert(
                String::from("value"),
                NP_Cursor::json_encode(0, &x, &self.memory),
            );
    ...
    pub fn move_cursor(&mut self, path: &[&str]) -> Result<bool, NP_Error> {
        let value_cursor =
            NP_Cursor::select(&self.memory, self.cursor.clone(), self.mutable, false, path)?;
    ...
    pub fn set_max(&mut self, path: &[&str]) -> Result<bool, NP_Error> {
        ...
        let value_cursor =
            NP_Cursor::select(&self.memory, self.cursor.clone(), self.mutable, false, path)?;
        match value_cursor {
            Some(x) => Ok(NP_Cursor::set_max(x, &self.memory)?),
            None => Ok(false),
        }
    }
    ...
    pub fn set_min(&mut self, path: &[&str]) -> Result<bool, NP_Error> {
        ...
        let value_cursor =
            NP_Cursor::select(&self.memory, self.cursor.clone(), self.mutable, false, path)?;
        match value_cursor {
            Some(x) => Ok(NP_Cursor::set_min(x, &self.memory)?),
            None => Ok(false),
        }
    }
    ...
    /// ```
    ///
    pub fn set<'set, X: 'set>(&mut self, path: &[&str], value: X) -> Result<bool, NP_Error>
    where
        ...
        match value_cursor {
            Some(x) => {
                ...
                match parsed["value"] {
                    ...
                    _ => {
                        NP_Cursor::set_from_json(
                            0,
                            false,
                            x,
                            &self.memory,
                            &Box::new(parsed["value"].clone()),
                        )?;
                    }
                }
        ...
        X: NP_Value<'push> + NP_Scalar<'push>,
    {
        ...
        let list_cursor = if path.len() == 0 {
            self.cursor.clone()
        } else {
            match NP_Cursor::select(&self.memory, self.cursor.clone(), true, false, path)? {
                Some(x) => x,
                None => return Ok(None),
            }
        };
            ...
            NP_TypeKeys::Bytes => {
                ...
                if size > 0 {
                    ...
                    let length_bytes = self
                        .memory
                        .get_4_bytes(addr_value as usize)
                        .unwrap_or(&[0u8; 4]);
    ...
    pub fn del(&mut self, path: &[&str]) -> Result<bool, NP_Error> {
        ...
        match value_cursor {
            Some(x) => NP_Cursor::delete(x, &self.memory),
            None => Ok(false),
        }
    }
    ...
    pub fn get_schema_type(&self, path: &[&str]) -> Result<Option<NP_TypeKeys>, NP_Error> {
        match NP_Cursor::select(&self.memory, self.cursor.clone(), false, true, path)? {
            Some(x) => Ok(Some(self.memory.get_schema(x.schema_addr).i)),
            None => Ok(None),
        }
    }
    ...
    /// ```
    ///
    pub fn get<'get, X: 'get>(&'get self, path: &[&str]) -> Result<Option<X>, NP_Error>
    where
        X: NP_Value<'get> + NP_Scalar<'get>,
    {
        let value_cursor =
        ...
        match value_cursor {
            Some(x) => {
                ...
                match X::into_value(&x, &self.memory)? {
                    Some(x) => Ok(Some(x)),
                    None => {
                        // no value found here, return default from schema
                        match X::default_value(0, x.schema_addr, &self.memory.get_schemas()) {
                            Some(y) => Ok(Some(y)),
                            None => {
                                // no default in schema, no value to provide
                                Ok(None)
                            }
                        }
                    }
    ...
    pub fn compact<'compact>(&mut self, new_capacity: Option<usize>) -> Result<(), NP_Error> {
        ...
        if self.memory.is_ref_mut() {
            let new_bytes = NP_Memory::new(
                capacity,
                self.memory.get_schemas() as *const Vec<NP_Parsed_Schema>,
                self.memory.root,
            );
    ...
    pub fn copy_buffer(&self) -> NP_Buffer {
        ...
        let new_memory = NP_Memory::existing_owned(
            copy_bytes,
            self.memory.get_schemas() as *const Vec<NP_Parsed_Schema>,
            self.memory.root,
        );
...
impl<'item> NP_Item<'item> {
    ...
    pub fn get<X>(&'item self) -> Result<Option<X>, NP_Error>
    where
        X: NP_Value<'item> + NP_Scalar<'item>,
    {
        if let Some(cursor) = self.cursor {
            match X::into_value(&cursor, self.memory)? {
                Some(x) => Ok(Some(x)),
                None => match X::default_value(0, cursor.schema_addr, &self.memory.get_schemas()) {
                    Some(y) => Ok(Some(y)),
                    None => Ok(None),
                },
            }
    ...
    pub fn set<X>(&'item mut self, value: X) -> Result<(), NP_Error>
    where
        X: NP_Value<'item> + NP_Scalar<'item>,
    {
        ...
        if let Some(cursor) = self.cursor {
            ...
            match schema.i {
                ...
                NP_TypeKeys::Struct => {
                    let item = opt_err(NP_Struct::select(
                        self.parent.clone(),
                        schema,
                        &self.key,
                        true,
                ...
                NP_TypeKeys::Tuple => {
                    let item = opt_err(NP_Tuple::select(
                        self.parent.clone(),
                        schema,
                        self.index,
                        true,
...
impl<'col> NP_Iterator_Collection<'col> {
    pub fn new(cursor: NP_Cursor, memory: &'col NP_Memory) -> Result<Self, NP_Error> {
        match memory.get_schema(cursor.schema_addr).i {
            ...
            _ => Err(NP_Error::new(
                "Tried to create iterator on non collection item!",
            )),
        }
    }
}
