...
use crate::{
    collection::{list::NP_List, map::NP_Map, struc::NP_Struct, tuple::NP_Tuple},
    schema::NP_TypeKeys,
};
use crate::{
    idl::{JS_Schema, JS_AST},
    pointer::dec::NP_Dec,
    schema::{NP_Portal_Data, NP_Schema_Addr},
    utils::opt_err,
};
use alloc::boxed::Box;
...
use self::{
    date::NP_Date, geo::NP_Geo, option::NP_Enum, portal::NP_Portal, ulid::NP_ULID, uuid::NP_UUID,
};
...
pub trait NP_Pointer_Bytes {
    ...
    fn get_addr_value(&self) -> u32 {
        0
    }
    fn set_addr_value(&mut self, addr: u32) {}
    fn get_next_addr(&self) -> u32 {
        0
    }
...
impl NP_Pointer_Bytes for NP_Pointer_Scalar {
    ...
    #[inline(always)]
    fn get_addr_value(&self) -> u32 {
        u32::from_be_bytes(self.addr_value)
    }
    #[inline(always)]
    fn set_addr_value(&mut self, addr: u32) {
        self.addr_value = addr.to_be_bytes()
    }
    #[inline(always)]
    fn reset(&mut self) {
        self.addr_value = [0; 4];
...
impl NP_Pointer_Bytes for NP_Pointer_List_Item {
    ...
    #[inline(always)]
    fn get_next_addr(&self) -> u32 {
        u32::from_be_bytes(self.next_value)
    }
    #[inline(always)]
    fn set_next_addr(&mut self, addr: u32) {
        self.next_value = addr.to_be_bytes()
    }
    #[inline(always)]
    fn set_index(&mut self, index: u16) {
        self.index = index.to_be_bytes()
    }
    #[inline(always)]
    fn get_index(&self) -> u16 {
...
impl NP_Pointer_Bytes for NP_Pointer_Map_Item {
    ...
    #[inline(always)]
    fn set_key_addr(&mut self, addr: u32) {
        self.key_addr = addr.to_be_bytes();
    }
    #[inline(always)]
    fn get_key_addr(&self) -> u32 {
        u32::from_be_bytes(self.key_addr)
    }
    #[inline(always)]
    fn reset(&mut self) {
        self.addr_value = [0; 4];
...
impl<'cursor> NP_Cursor {
    ...
    pub fn get_value(&self, memory: &NP_Memory) -> &'cursor dyn NP_Pointer_Bytes {
        ...
        if self.buff_addr == memory.root || self.buff_addr > memory.read_bytes().len() {
            ...
            match memory.get_schema(self.parent_schema_addr).i {
                NP_TypeKeys::List => unsafe {
                    &*(ptr.add(self.buff_addr) as *const NP_Pointer_List_Item)
                },
                NP_TypeKeys::Map => unsafe {
                    &*(ptr.add(self.buff_addr) as *const NP_Pointer_Map_Item)
                },
                NP_TypeKeys::Tuple => match &self.value_bytes {
    ...
    pub fn get_value_mut(&self, memory: &NP_Memory) -> &'cursor mut dyn NP_Pointer_Bytes {
        ...
        if self.buff_addr == memory.root || self.buff_addr > memory.read_bytes().len() {
            ...
            match memory.get_schema(self.parent_schema_addr).i {
                NP_TypeKeys::List => unsafe {
                    &mut *(ptr.add(self.buff_addr) as *mut NP_Pointer_List_Item)
                },
                NP_TypeKeys::Map => unsafe {
                    &mut *(ptr.add(self.buff_addr) as *mut NP_Pointer_Map_Item)
                },
                NP_TypeKeys::Tuple => match &self.value_bytes {
    ...
    pub fn select(
        ...
        schema_query: bool,
        path: &[&str],
    ) -> Result<Option<NP_Cursor>, NP_Error> {
        ...
        loop {
            ...
            match schema.i {
                NP_TypeKeys::Struct => {
                    if let Some(next) = NP_Struct::select(
                        loop_cursor,
                        schema,
                        path[path_index],
                        make_path,
                ...
                NP_TypeKeys::Tuple => match path[path_index].parse::<usize>() {
                    Ok(x) => {
                        if let Some(next) = NP_Tuple::select(
                            loop_cursor,
                            schema,
                            x,
                            make_path,
                    ...
                    Err(_e) => {
                        return Err(NP_Error::new(
                            "Need a number to index into tuple, string found!",
                        ))
                    }
                },
                NP_TypeKeys::List => match path[path_index].parse::<usize>() {
                    Ok(x) => {
                        if let Some(next) =
                            NP_List::select(loop_cursor, x, make_path, schema_query, memory)?
                        {
                    ...
                    Err(_e) => {
                        return Err(NP_Error::new(
                            "Need a number to index into list, string found!",
                        ))
                    }
                },
                NP_TypeKeys::Map => {
                    if let Some(next) = NP_Map::select(
                        loop_cursor,
                        path[path_index],
                        make_path,
                        schema_query,
                ...
                _ => {
                    // we've reached a scalar value but not at the end of the path
                    return Ok(None);
                }
            }
        }
        ...
        match memory.get_schema(cursor.schema_addr).i {
            NP_TypeKeys::Boolean => {
                bool::set_value(
                    cursor,
                    memory,
                    opt_err(bool::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::UTF8String => {
                String::set_value(
                    cursor,
                    memory,
                    opt_err(String::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Bytes => {
                NP_Bytes::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Bytes::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Int8 => {
                i8::set_value(cursor, memory, opt_err(i8::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int16 => {
                i16::set_value(cursor, memory, opt_err(i16::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int32 => {
                i32::set_value(cursor, memory, opt_err(i32::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int64 => {
                i64::set_value(cursor, memory, opt_err(i64::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint8 => {
                u8::set_value(cursor, memory, opt_err(u8::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint16 => {
                u16::set_value(cursor, memory, opt_err(u16::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint32 => {
                u32::set_value(cursor, memory, opt_err(u32::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint64 => {
                u64::set_value(cursor, memory, opt_err(u64::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Float => {
                f32::set_value(cursor, memory, opt_err(f32::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Double => {
                f64::set_value(cursor, memory, opt_err(f64::np_max_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Decimal => {
                NP_Dec::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Dec::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Geo => {
                NP_Geo::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Geo::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Date => {
                NP_Date::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Date::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Enum => {
                NP_Enum::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Enum::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Uuid => {
                NP_UUID::set_value(
                    cursor,
                    memory,
                    opt_err(NP_UUID::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Ulid => {
                NP_ULID::set_value(
                    cursor,
                    memory,
                    opt_err(NP_ULID::np_max_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Struct => {
        ...
        match memory.get_schema(cursor.schema_addr).i {
            NP_TypeKeys::Boolean => {
                bool::set_value(
                    cursor,
                    memory,
                    opt_err(bool::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::UTF8String => {
                String::set_value(
                    cursor,
                    memory,
                    opt_err(String::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Bytes => {
                NP_Bytes::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Bytes::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Int8 => {
                i8::set_value(cursor, memory, opt_err(i8::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int16 => {
                i16::set_value(cursor, memory, opt_err(i16::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int32 => {
                i32::set_value(cursor, memory, opt_err(i32::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Int64 => {
                i64::set_value(cursor, memory, opt_err(i64::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint8 => {
                u8::set_value(cursor, memory, opt_err(u8::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint16 => {
                u16::set_value(cursor, memory, opt_err(u16::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint32 => {
                u32::set_value(cursor, memory, opt_err(u32::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Uint64 => {
                u64::set_value(cursor, memory, opt_err(u64::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Float => {
                f32::set_value(cursor, memory, opt_err(f32::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Double => {
                f64::set_value(cursor, memory, opt_err(f64::np_min_value(&cursor, memory))?)?;
            }
            NP_TypeKeys::Decimal => {
                NP_Dec::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Dec::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Geo => {
                NP_Geo::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Geo::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Date => {
                NP_Date::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Date::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Enum => {
                NP_Enum::set_value(
                    cursor,
                    memory,
                    opt_err(NP_Enum::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Uuid => {
                NP_UUID::set_value(
                    cursor,
                    memory,
                    opt_err(NP_UUID::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Ulid => {
                NP_ULID::set_value(
                    cursor,
                    memory,
                    opt_err(NP_ULID::np_min_value(&cursor, memory))?,
                )?;
            }
            NP_TypeKeys::Struct => {
                let mut struc = NP_Struct::new_iter(&cursor, memory);
    ...
    pub fn compact(
        depth: usize,
        from_cursor: NP_Cursor,
        from_memory: &NP_Memory,
        to_cursor: NP_Cursor,
        to_memory: &NP_Memory,
        ...
        match from_memory.get_schema(from_cursor.schema_addr).i {
            ...
            NP_TypeKeys::UTF8String => {
                String::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Bytes => {
                NP_Bytes::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Int8 => {
                i8::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Int16 => {
                i16::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Int32 => {
                i32::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Int64 => {
                i64::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Uint8 => {
                u8::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Uint16 => {
                u16::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Uint32 => {
                u32::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Uint64 => {
                u64::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Float => {
                f32::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Double => {
                f64::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Decimal => {
                NP_Dec::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Boolean => {
                bool::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Geo => {
                NP_Geo::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Uuid => {
                NP_UUID::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Ulid => {
                NP_ULID::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Date => {
                NP_Date::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Enum => {
                NP_Enum::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Struct => {
                NP_Struct::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Map => {
                NP_Map::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::List => {
                NP_List::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Tuple => {
                NP_Tuple::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
            NP_TypeKeys::Portal => {
                NP_Portal::do_compact(depth, from_cursor, from_memory, to_cursor, to_memory)
            }
    ...
    pub fn set_schema_default(cursor: NP_Cursor, memory: &NP_Memory) -> Result<(), NP_Error> {
        ...
        match schema.i {
            NP_TypeKeys::None => {
                return Err(NP_Error::Unreachable);
            }
            NP_TypeKeys::Any => {
                return Err(NP_Error::Unreachable);
            }
            ...
            NP_TypeKeys::Portal => {
                return Err(NP_Error::new("Portal type does not have a default type"));
            }
            // NP_TypeKeys::Union       => { return Err(NP_Error::new("Union type does not have a default type")); },
            NP_TypeKeys::UTF8String => {
                String::set_value(cursor, memory, opt_err(String::schema_default(schema))?)?;
            }
            NP_TypeKeys::Bytes => {
                NP_Bytes::set_value(cursor, memory, opt_err(NP_Bytes::schema_default(schema))?)?;
            }
            NP_TypeKeys::Int8 => {
                i8::set_value(cursor, memory, opt_err(i8::schema_default(schema))?)?;
            }
            NP_TypeKeys::Int16 => {
                i16::set_value(cursor, memory, opt_err(i16::schema_default(schema))?)?;
            }
            NP_TypeKeys::Int32 => {
                i32::set_value(cursor, memory, opt_err(i32::schema_default(schema))?)?;
            }
            NP_TypeKeys::Int64 => {
                i64::set_value(cursor, memory, opt_err(i64::schema_default(schema))?)?;
            }
            NP_TypeKeys::Uint8 => {
                u8::set_value(cursor, memory, opt_err(u8::schema_default(schema))?)?;
            }
            NP_TypeKeys::Uint16 => {
                u16::set_value(cursor, memory, opt_err(u16::schema_default(schema))?)?;
            }
            NP_TypeKeys::Uint32 => {
                u32::set_value(cursor, memory, opt_err(u32::schema_default(schema))?)?;
            }
            NP_TypeKeys::Uint64 => {
                u64::set_value(cursor, memory, opt_err(u64::schema_default(schema))?)?;
            }
            NP_TypeKeys::Float => {
                f32::set_value(cursor, memory, opt_err(f32::schema_default(schema))?)?;
            }
            NP_TypeKeys::Double => {
                f64::set_value(cursor, memory, opt_err(f64::schema_default(schema))?)?;
            }
            NP_TypeKeys::Decimal => {
                NP_Dec::set_value(cursor, memory, opt_err(NP_Dec::schema_default(schema))?)?;
            }
            NP_TypeKeys::Boolean => {
                bool::set_value(cursor, memory, opt_err(bool::schema_default(schema))?)?;
            }
            NP_TypeKeys::Geo => {
                NP_Geo::set_value(cursor, memory, opt_err(NP_Geo::schema_default(schema))?)?;
            }
            NP_TypeKeys::Uuid => {
                NP_UUID::set_value(cursor, memory, opt_err(NP_UUID::schema_default(schema))?)?;
            }
            NP_TypeKeys::Ulid => {
                NP_ULID::set_value(cursor, memory, opt_err(NP_ULID::schema_default(schema))?)?;
            }
            NP_TypeKeys::Date => {
                NP_Date::set_value(cursor, memory, opt_err(NP_Date::schema_default(schema))?)?;
            }
            NP_TypeKeys::Enum => {
                NP_Enum::set_value(cursor, memory, opt_err(NP_Enum::schema_default(schema))?)?;
            }
        }
    ...
    pub fn set_from_json(
        ...
        match memory.get_schema(cursor.schema_addr).i {
            ...
            NP_TypeKeys::Any => Ok(()),
            NP_TypeKeys::UTF8String => {
                String::set_from_json(depth, apply_null, cursor, memory, json)
            }
            ...
            NP_TypeKeys::Struct => {
                NP_Struct::set_from_json(depth, apply_null, cursor, memory, json)
            }
            ...
            NP_TypeKeys::Portal => {
                NP_Portal::set_from_json(depth, apply_null, cursor, memory, json)
            }
    ...
    pub fn calc_size(
        ...
        if depth > 255 {
            return Err(NP_Error::new("Depth error!"));
        }
        ...
        // size of pointer
        let base_size = if type_key == NP_TypeKeys::Portal {
            0
        } else {
            value.get_size()
        };
...
pub trait NP_Value<'value> {
    ...
    ///
    fn into_value(_cursor: &NP_Cursor, _memory: &'value NP_Memory) -> Result<Option<Self>, NP_Error>
    where
        Self: Sized,
    {
    ...
    fn default_value(
        depth: usize,
        scham_addr: usize,
        schema: &'value Vec<NP_Parsed_Schema>,
    ) -> Option<Self>
    where
        Self: Sized;
    ...
    fn do_compact(
        _depth: usize,
        from_cursor: NP_Cursor,
        from_memory: &'value NP_Memory,
        to_cursor: NP_Cursor,
        to_memory: &'value NP_Memory,
    ) -> Result<NP_Cursor, NP_Error>
    where
        Self: 'value + Sized,
    {
